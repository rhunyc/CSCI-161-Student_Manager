...
public class CourseFactory implements Serializable {
    ...
    private static ArrayList<Course> prereq = new ArrayList<>();
    ...
    public static void generateCourse()
    {
        ...
        int daysL = rand.nextInt((5 - 2) + 2 ) + 1;
        days = new int[daysL];
        for(int x = 0; x < days.length; x++)
        {
            days[x] = rand.nextInt((5 - 2) + 2 ) + 1;
        }
        time = times[rand.nextInt(times.length)];
        capacity = rand.nextInt((50 - 20) + 20) + 1;
        
        credits = rand.nextInt((5 - 2) + 2) + 1;
...
//        if(!Administrator.accessCourse().isEmpty())
//        {
//            int totalPre = rand.nextInt(3);
//            for(int x = 0; x < totalPre; x++)
//            {
//                prereq.add(Administrator.accessCourse().get(rand.nextInt(Administrator.accessCourse().size())));
//            }
//        }
        
        
        Course c = new Course(name, days, time, major, prereq, capacity, teach, credits);
    }
    /*
    public Course(String name, int[] days, int time, String major, ArrayList<Course> 
            Prereq, int Capacity, String[] materialsNeeded, String teacher, ArrayList<Student> enrolled, int credits, int courseID)
    */
